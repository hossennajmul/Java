...
import javax.swing.*;
import java.sql.*;
public class SearchDoctorBySpecialityFrame extends JFrame implements ActionListener {
	...
	public SearchDoctorBySpecialityFrame()
	{
		this.initialize();
	}
	...
	public void actionPerformed(ActionEvent event)
	{
		...
		try
		{
			...
			PreparedStatement st = con.prepareStatement("Select * from Doctor where specialty =?");
			st.setString(1,str);
			rs=st.executeQuery();
			if(rs.next())
			{
	...
	private void initialize()
	{
		...
		this.labelsearch = new JLabel("Enter Speciality");
		...
		this.add(this.panel);
		this.setTitle("Search Doctor By Speciality");
